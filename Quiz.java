...
public class Quiz implements ActionListener {
    ...
    int index;
    int correctGuess;
    int totalQuestion = Questions.length;// string me jitny question hy us hisab sy length adjest hogi
    int results;
    int Seconds=10;
    ...
    JTextField  textfield = new JTextField();// this will hold the current text field(this allow to enter single text line)
...
//ABOVE ALL ARE ACTING AS GLOBAL VARIABLES SO WE CAN ACCESS THESE ANYWHERE WITHIN OUR PROGRAM

Timer timerForQuestion = new Timer(1000,new ActionListener(){ // when the timer reaches 1 seconds we gonna performed wheter is in this action performed method
    ...
    public Quiz() // constructor
    {
       ...
       textfield.setBounds(0,0,650,50);//(x cor, y cor,length,heigth ) //determines where the text field is gonna be placed as well as the heigth and the width 
      ...
      // textfield.setText("testing");
       ...
       textArea.setBounds(0,50,650,50);//(x cor, y cor,length,heigth ) //determines where the text field is gonna be placed as well as the heigth and the width 
       textArea.setLineWrap(true); // IN CASE IF TEXT GOES OF THE SCREEN OR TEXT AREA ITS GOING TO AROUND TO NEXTLINE
   ...
   //    textArea.setText("QUESTIONS");
      ...
      buttonA.setFocusable(false); //FOCUSES THE TEXT COMPONENT WITHIN OUR BUTTON THIS IS TRUE BY DEFAULT IF WE DONT WANT IT THAN WE SET IT FALSE
      ...
      answer_labelA.setBackground(new Color(50,50,50)); // Color.grey for default color for your personalized color use(new Color(R,G,B))
     ...
     seconds_left.setBorder(BorderFactory.createBevelBorder(1)); //setting border from border factory createbevelborder is our border type and 1 is bevelborder.Raised and 0 is bevelborder.LOWER
 ...
 //for showing percentage at the end of the quiz
    ...
    public void displayAnswer(){
      ...
      if (answers[index]!='A'){  // if our answer is wrong our all answer will be labeled as red except the right answer.
    ...
    // now adding a 2 second delay after changing color into red changing it back to green 
     
    Timer pause = new Timer(1000,new ActionListener(){ // when the timer reaches 2 seconds we gonna performed wheter is in this action performed method
     // @Override ??///(Declaring a method in sub class which is already present in parent class is known as method overriding. Overriding is done so that a child class can give its own implementation to a method which is already provided by the parent class)
     // action performed hamrari action listener ki calss me already hy we are using this agian for custamized implemntation
      public void actionPerformed(ActionEvent e){
        ...
        buttonA.setEnabled(true);  // to reenabel our buttons if we dont enabelled it see the outoput in your gallery
    ...
    pause.start();
    pause.setRepeats(false); //(our timer) will only execute once (what ever is in our action performed method)
    ...
    public void results(){
         ...
         buttonA.setEnabled(false); // disabling the button so user wiil not be able to press the button after the quiz is over
         buttonC.setEnabled(false);
         buttonB.setEnabled(false);
         buttonD.setEnabled(false);
